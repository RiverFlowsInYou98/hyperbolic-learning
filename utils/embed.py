...
my_path = (
    "C:\\Users\\dreww\\Desktop\\hyperbolic-learning\\utils"  # path to utils folder
)
...
def evaluate_model(
    ...
    if report:
        ...
        if classifier == "hsvm":
            ...
            y_pred = model.predict(
                poincare_pts_to_hyperboloid(X_test, metric="minkowski")
            )
        elif classifier == "hgmm":
            model.fit(
                poincare_pts_to_hyperboloid(X_train, metric="minkowski"),
                y_train,
    ...
    for train, test in kf.split(X):
        if classifier == "hsvm":
            model.fit(
                poincare_pts_to_hyperboloid(X[train], metric="minkowski"), y[train]
            )
            y_pred = model.predict(
                poincare_pts_to_hyperboloid(X[test], metric="minkowski")
            )
        elif classifier == "hgmm":
            model.fit(
                poincare_pts_to_hyperboloid(X[train], metric="minkowski"),
                y[train],
                max_epochs=max_epochs,
                alpha=alpha,
            )
